...
def uploadquestionpaper():
	...
	return jsonify({"success":"true","qplink":"http://espl.in.net/mt/"+destination})
...
def uploadTimetableportion():
	...
	return jsonify({"success":"true","ttportion":"http://espl.in.net/mt/"+destination})
...
def uploadTimetable():
	...
	return jsonify({"success":"true","ttlink":"http://espl.in.net/mt/"+destination})
...
def getAdmittedStudentList():
	if request.get_json():
		if request.get_json()["batch"]=="true":
			batchname = request.get_json()["batchname"]
			data = mongo.db.student_info.find({"Batch":str(batchname)})
		else:
			idi = request.get_json()["id"]
			data = mongo.db.student_info.find({"_id":ObjectId(str(idi))})
	else:
		data = mongo.db.student_info.find({"admitStatus": 1})
	studedata = list(data)
	for stud in studedata:
		stud["_id"]=str(stud["_id"])
