...
def getstudentslistforattendance():
	if request.get_json():
		if request.get_json()["batch"]=="true":
			...
			data = mongo.db.student_info.find({"Batch":str(batchname),"isDeleted":"false"},{"firstName":True,
		"middleName":True,
		"lastName":True,
		"RollNo":True,
		"studentMobile":True,
		"_id":True})
			...
			idi = request.get_json()["id"]
			data = mongo.db.student_info.find({"_id":ObjectId(str(idi)),"isDeleted":"false"},{"firstName":True,
		"middleName":True,
		"lastName":True,
		"RollNo":True,
		"studentMobile":True,
		...
		data = mongo.db.student_info.find({"admitStatus": 1 , "isDeleted":"false"},{"firstName":True,
		"middleName":True,
		"lastName":True,
		"RollNo":True,
		"studentMobile":True,
...
def getStaffInfo():
	idi = request.get_json()["id"]
	data = mongo.db.staffdetails.find({"_id":ObjectId(str(idi)),"isDeleted":"false"})
	staffdata = list(data)
	if(len(staffdata)):
...
def getemployeelist():
	staff = mongo.db.staffdetails.find({"cityid":str(request.get_json()["cityid"]),"stafftype":"Employee","isDeleted":"false"})
	staffs = list(staff)
...
def addemployeeattendance():
	cardid=mongo.db.staffRfidlist.find_one({"staffid":request.get_json()["staffid"]},{"cardid":True})
	if(cardid):
		request.get_json()["cardid"]=cardid["cardid"]
		...
		return jsonify({"success":"false","message":"Please Assign Rfid Card To Staff"})
...
def getlecturedetails():
	...
	lecturedetails=mongo.db.staffattendancedetails.find({"batch":batch,"isDeleted":"false"}).distinct("date")
	...
	if len(lecturedetails):
		...
		for day in d:
			...
			lectures=mongo.db.staffattendancedetails.find({"batch":batch,"date":day,"isDeleted":"false"},{"_id":False})
			lectures=list(lectures)
			for l in lectures:
				l["date"]=day
...
def getcoursesnew():
 	if(request.get_json()):
		...
		courses = mongo.db.coursedetails.find({})	
	...
	if len(course):
	 	return jsonify({"success":"true","courses":course})
	else:
...
def getbatchattendance():
	...
	batchname=request.get_json()["batchname"]
	s=[]
	students=mongo.db.student_info.find({"Batch":batchname,"isDeleted":"false"},{"_id":True,"firstName":True,"lastName":True,"RollNo":True}).sort('RollNo',1);
	students=list(students)
	if len(students):
...
def markEmpAttendance():
	...
	staffinfo=mongo.db.staffRfidlist.find_one({"staffid":request.get_json()["id"]})
	if(staffinfo):
		mongo.db.staffRfidattendance.update({"cardid":staffinfo["cardid"],"staffid":request.get_json()["id"],"date":request.get_json()["date"]},{'$push': {'time': request.get_json()["time"]}}, upsert = True)	
		return jsonify({"success":"true","message":"Attendance Marked"})
	else:
		return jsonify({"success":"false","message":"Please Assign Rfid Card"})
...
@app.route("/getfeedbackstatus",methods=["GET"])
def getfeedbackstatus():
	feedbackbutton=mongo.db.feedbackbutton.find_one({},{"_id":False})	
	if len(feedbackbutton):
		return jsonify({"success":"true","feedbackbutton":feedbackbutton})
	else:
		return jsonify({"success":"false"})

@app.route("/getfeedbacksbybranch",methods=["POST"])
def getfeedbacksbybranch():
	if len(request.get_json()):
		print "in if"

		batchd=mongo.db.branches.find_one({"_id":ObjectId(request.get_json()["branchid"])},{"_id":False,"branchname":True})	
		batchname=batchd["branchname"]
		print batchname
		feedbacks=mongo.db.feedback.find({"name_branch":batchname,"approve":1},{"_id":False})	
		...
		feedbacks=mongo.db.feedback.find({"approve":1},{"_id":False})	
		feedbacks=list(feedbacks)
		if len(feedbacks):
			return jsonify({"success":"true","feedbacks":feedbacks})
		else:
...
if __name__ == "__main__":
	app.run(debug=True, host='0.0.0.0', port=7454, threaded=True,ssl_context= 'adhoc')